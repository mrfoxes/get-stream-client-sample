...
type Props = {
  ...
  paginated: Boolean,
  itemsPerPage: Number,
  emptyFeedComponent: Element | Function,
...
class FeedStreamer extends Component < Props > {
    ...
    buildOptions = () => {
      ...
      if (paginated) {
        Object.assign(cleanedOptions, {
          limit: itemsPerPage,
          id_lt: this.getLatestFeedId()
        })
      }
    ...
    renderActivities = () => {
      ...
      return ( <
        ActivityList feeds = {
          this.filterFeed(feedStream)
        } >
        {
          isUndefined(component) ? < BaseActivityItem { ...extraProps
          }
          /> : safeRender(component, extraProps)} <
          /ActivityList>
        )
      }
